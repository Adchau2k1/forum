...
const handleClickDelete = (item) => {
    ...
    selectedValues._id = item._id
    selectedValues.topicId = item.topicId
}

const handleDelete = async () => {
    try {
        const { data: resDelete } = await restAPI.user.deletePost({
            body: {
                ...selectedValues,
            },
        })
                ...
                <template v-slot:footer.prepend>
                    <v-dialog v-model="dialogDelete" width="400px" height="120px">
                        <v-card class="h-full pt-2">
                            <v-card-title> Xác nhận xóa bài viết? </v-card-title>
